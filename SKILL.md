...
1. **理科五科「互动实验室」新模式——内容动态化 + 动态互动（2026-09-15，第 17 轮，推倒第 15 轮的理科辅导页形态）**：
   ...
   - **数据层**：Lesson 新增 `lab?: {params[], grid?, explore?, code?}`（LabDef/LabParam 类型）；**双轨覆盖**——94 节五科代码课全部进实验室：手工 lab 三批共 46 节（`scripts/labs-batch1/2/3.json` 数据 + `scripts/inject-labs.mjs labs-batchN.json` 注入器，幂等可加批次）：数学 10（一次/二次/正弦/方差/复利/方程组/勾股/反比例/指数爆炸/锐角三角/导数切线中选）物理 11（匀变速/牛二/浮力/平抛/杠杆/欧姆/自由落体/比热容/压强/简谐/机械能/万有引力/惯性滑行/串并联/回声测距/凸透镜/功功率中选）化学 6（平衡/溶解度/pH/周期律/配平检查/质量守恒）生物 4（孟德尔/显隐性/DNA 突变/光合作用）地理 4（时区/公转/大气受热/等高线）；**其余自动轨**——LabScreen 正则提取顶层「带 # 注释的数值赋值」为滑块，且**自动排除循环内自增的累加器**（`c0 = c0 + 1` 类计数器不当参数）
