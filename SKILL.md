...
1. **理科五科「互动实验室」新模式——内容动态化 + 动态互动（2026-09-15，第 17 轮，推倒第 15 轮的理科辅导页形态）**：
   ...
   - **数据层**：Lesson 新增 `lab?: {params[], grid?, explore?, code?}`（LabDef/LabParam 类型）；**双轨覆盖**——94 节五科代码课全部进实验室：手工 lab 第一批 17 节 + 第二批 14 节（`scripts/labs-batch1/2.json` 数据 + `scripts/inject-labs.mjs labs-batchN.json` 注入器，幂等可加批次）：数学 7（一次函数/抛物线/二次最值/正弦波/方差/复利曲线/方程组交点）物理 8（匀变速/牛二/浮力/平抛/杠杆/欧姆定律/自由落体/比热容/压强/简谐/机械能/万有引力中选）化学 4（平衡/溶解度/pH/周期律聚类）生物 2（孟德尔/显隐性）地理 3（时区/公转/大气受热）+ 影子测高；**其余自动轨**——LabScreen 正则提取顶层「带 # 注释的数值赋值」为滑块，且**自动排除循环内自增的累加器**（`c0 = c0 + 1` 类计数器不当参数，第二批修复）
