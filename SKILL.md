...
1. **AI+学习模式（2026-09-07，第 13 轮，对标 Khanmigo/Duolingo）**：
   - 【兴趣】每日任务条（通关 1 课/答对 5 题/交流 2 次，服务端自动累计跨天重置）+ 🔥连续天数 + ⚡XP 五级称号（见习探索者→传奇创造者，通关+50/答题+10/交流+5）
   - 【理解】AIBuddy 快捷追问条（🤔为什么/🔄换个说法/🌰生活例子/🌉知识连接/🎯考考我）+ prompts 配套 Khanmigo 式应答；通关复盘三问（元认知，AI 主动发起）
   - 【广度】通关庆祝「下一步探险」推荐（同学科+知识点重叠+相邻年级加权取 2）
   - 服务端：ProfileProgress.xp/questDate/questDone；mergeProgress 任务引擎（API 实测 50→80→85 递增正确）；addQuestChat；PIN 删除防护实测（403 拒绝无 PIN）
