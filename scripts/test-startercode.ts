// 用项目自身的 Python 解释器对全部代码课 starterCode 做真实解析测试
// 用法: npx tsx scripts/test-startercode.ts
...
for (const f of readdirSync(dir).filter(x => x.endsWith('.json'))) {
  ...
  if (!d.codeLesson || !d.starterCode) continue;
  const r = parsePy(d.starterCode);
  if (r.error) { fail++; failures.push(`${f} [行${r.error.line}] ${r.error.message}${r.error.hint ? ' | ' + r.error.hint : ''}`); }
  else pass++;
}
console.log(`代码课解析测试: ${pass} 通过 / ${fail} 失败`);
