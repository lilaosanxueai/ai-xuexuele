...
export function buildRouter(cfg: AppConfig): Router {
  ...
  r.put('/progress/:profileId', (req, res) => {
    const { lessonId, tasks, completed, minutesDelta, draft, code, exercise, wrongAdds, wrongClears } = req.body ?? {};
    ...
    res.json(store.mergeProgress(req.params.profileId, { lessonId, tasks, completed, minutesDelta, draft, code, exercise, wrongAdds, wrongClears }));
  });
